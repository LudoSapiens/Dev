...
GravitationalAttractor::addForce( const Vector< RCP<RigidBody> >& bodies )
{
   ...
   {
      DBG_MSG( os_att, "Body[" << b << "]: mass=" << bodies[b]->mass() << " position=" << bodies[b]->centerPosition() );
   }
   ...
   for( uint a = 0; a < bodies.size(); ++a )
   {
      ...
      {
         Vec3f forceAtoB = bodies[b]->centerPosition() - bodies[a]->centerPosition();
         float rSq = forceAtoB.sqrLength();
         float GMm_rr = Constants::G * bodies[a]->mass() * bodies[b]->mass() / rSq;
         DBG_MSG( os_att, "Ma:" << bodies[a]->mass() << " Pa: " << bodies[a]->centerPosition() );
         DBG_MSG( os_att, "Mb:" << bodies[b]->mass() << " Pb: " << bodies[b]->centerPosition() );
