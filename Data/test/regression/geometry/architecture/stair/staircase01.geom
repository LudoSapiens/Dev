...
for c in query( "centerB" ) do
   split( c, "X", { id="stairB", sw }, { id="emptyB", rel=1 }, { id="empty2B", sw } )
end

for c in query( "centerT" ) do
   split( c, "X", { id="empty2T", sw }, { id="emptyT", rel=1 }, { id="stairT", sw, orientation=5 } )
end
...
if stw > 0 then
   ...
   for c in query( "landingWA" ) do
      if c.level == lastLevel then
         split( c, "X", { id="landingWS", sw-stw }, { id="landingW", rel=1 } )
      else
