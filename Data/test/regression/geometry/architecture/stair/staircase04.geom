...
for c in query( "centerB" ) do
   split( c, "X", { id="stairB", rel=1 }, { id="emptyB", ew }, { id="empty2B", rel=2 }, { id="emptyB", ew }, { id="stairB", rel=1 } )
end

for c in query( "centerT" ) do
   split( c, "X", { id="empty2T", rel=1 }, { id="emptyT", ew }, { id="stairT", rel=2, orientation=5 }, { id="emptyT", ew }, { id="empty2T", rel=1 } )
end
...
if stw > 0 then
   ...
   for c in query( "landingWA" ) do
      if c.level == lastLevel then
         split( c, "X", { id="landingW", rel=1 }, { id="landingW", ew+2*stw }, { id="landingWS", rel=2 }, { id="landingW", ew+2*stw }, { id="landingW", rel=1 } )
      else
