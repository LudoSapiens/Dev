--==============================================================================
-- Simple W-shape staircase with 2 landing
--==============================================================================
...
for c in query( "centerB" ) do
   split( c, "X", { id="empty2B", rel=1 }, { id="emptyB", ew }, { id="stairB", rel=2 }, { id="emptyB", ew }, { id="empty2B", rel=1 } )
end

for c in query( "centerT" ) do
   split( c, "X", { id="stairT", rel=1 , orientation=5}, { id="emptyT", ew }, { id="empty2T", rel=2 }, { id="emptyT", ew }, { id="stairT", rel=1, orientation=5 } )
end
...
if stw > 0 then
   ...
   for c in query( "landingWA" ) do
      if c.level == lastLevel then
         split( c, "X", { id="landingWS", rel=1 }, { id="landingW", ew+2*stw }, { id="landingW", rel=2 }, { id="landingW", ew+2*stw }, { id="landingWS", rel=1 } )
      else
         split( c, "X", { id="landingWS", rel=1 }, { id="landingW", ew+2*stw }, { id="landingWS", rel=2 }, { id="landingW", ew+2*stw }, { id="landingWS", rel=1 } )
      end
   end
