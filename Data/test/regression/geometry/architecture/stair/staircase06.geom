...
for c in query( "staircaseLevel" ) do
   if c.level == lastLevel then
      split( c, "X", { id="emptyB", rel=1 }, { id="bottomLanding", sw } ) 
   else
...
for c in query( "qLevel1" ) do
   split( c, "X", { id="emptyB", rel=1 }, { id="lstairl", sw } )
end
for c in query( "qLevel2" ) do
   split( c, "Z", { id="lstairl", sw, orientation=4 }, { id="emptyB", rel=1 } )
end
for c in query( "qLevel3" ) do
   split( c, "X", { id="lstairl", sw, orientation=5 }, { id="emptyB", rel=1 } )
end
for c in query( "qLevel4" ) do
   split( c, "Z", { id="emptyB", rel=1 }, { id="lstairl", sw, orientation=1 } )
end
